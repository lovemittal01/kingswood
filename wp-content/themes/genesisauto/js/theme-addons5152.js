                ...
                if (appendElement) {
                    ...
                    if ( e.relatedTarget ) {
                        ...
                        if (newTarget.length > 0 && newTarget.hasClass('wgl-cursor-text')) {
                            ...
                            cursorPointer.css('color','var(--genesisauto-cursor-point-color)')
                        }
					}
                }
            })
        ...
        jQuery(this).appear(function() {
            ...
            if (item_appear.length) {
                item_appear.each(function() {
                    jQuery(this).addClass('show_divider');
                });
            }
        });
    });
...
function wglDynamicStyles() {
    var style = jQuery('#genesisauto-footer-inline-css');
...
function wglTextEditor() {
    ...
    if ($mask_effect.length) {
        $mask_effect.each(function () {
            var wrapper = jQuery(this).find('.text-editor_wrapper'),
                firstElement = wrapper.find('.text-editor_wrapper-inner'),
                secondElement = firstElement.nextAll('.text-editor_wrapper-inner').eq(0);

            if (wrapper.find('.text-editor_wrapper-inner-clone').length === 0) {
                jQuery(secondElement).addClass('text-editor_wrapper-clone').wrap('<div class="text-editor_outer-clone"></div>');
            }
            var cloneOuter = wrapper.find('.text-editor_outer-clone'),
                cloneText = wrapper.find('.text-editor_wrapper-clone');

            var init = function () {
                var textMaskWidth = wrapper.outerWidth();
                cloneText.css('width', textMaskWidth);
            }
